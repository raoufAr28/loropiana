    ...
    <section className="mt-24 pt-16 border-t border-gray-100 dark:border-white/5">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="glass-panel-heavy rounded-[2.5rem] p-8 md:p-12 border border-white/10 dark:border-white/5 shadow-2xl relative overflow-hidden group">
          ...
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#00487A]/5 blur-[100px] rounded-full -translate-x-1/2 -translate-y-1/2 pointer-events-none" />
              ...
              <div className="flex gap-1 mb-6">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star 
                    key={s} 
                    size={24} 
                    fill={s <= Math.round(rating) ? LUXURY_BLUE : "transparent"} 
                    className={s <= Math.round(rating) ? "text-[#00487A]" : "text-gray-200 dark:text-gray-800"} 
                  />
                ))}
              </div>
                ...
                <div key={star} className="flex items-center gap-4 group/row">
                  ...
                  <div className="flex-1 h-2 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden relative">
                    <motion.div 
                      ...
                      className="absolute top-0 bottom-0 left-0 rounded-full shadow-[0_0_15px_rgba(0,72,122,0.3)]"
                      style={{ backgroundColor: LUXURY_BLUE }}
                    />
                  </div>
                  
                  <span className="text-[10px] font-bold text-taupe w-10 text-right opacity-50 group-hover/row:opacity-100 transition-opacity">
                    {distribution[star]}%
                  </span>
                </div>
              ))}
