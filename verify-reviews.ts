...
async function verify() {
  ...
  try {
    ...
    if (schemaError) throw new Error(`Schema error: ${schemaError.message}. Did you run the SQL migration?`);
    console.log('   Schema OK. Columns:', Object.keys(sample[0] || {}).join(', '));

    // 2. Submit Review
    console.log('2. Submitting test review...');
    const testReview = {
      full_name: 'Verification Bot ' + Date.now(),
    ...
    console.log('4. Verifying public fetch (GET /api/reviews behavior)...');
    ...
    const isVisible = approved.some((r: any) => r.id === inserted.id);
    if (isVisible) {
